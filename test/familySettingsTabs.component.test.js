...
//
// The settings dialog is five tab panels behind one sidebar. Two things about
...
describe('the settings sidebar is a real tablist', () => {
  ...
  it('points each tab at the panel it controls, and the panel back at it', async () => {
    const wrapper = mountSettings()
    await tabs(wrapper).find((t) => t.text().includes('About')).trigger('click')
...
describe('which tabs a viewer gets', () => {
  it('gives an owner every tab, with About last', () => {
    const labels = tabLabels(mountSettings())
    expect(labels).toEqual(['Overview', 'Preferences', 'Members', 'Danger Zone', 'About'])
  })
  ...
  it('falls back to Overview when the current tab stops existing', async () => {
    const wrapper = mountSettings({ initialTab: 'family' })
...
describe('panel routing', () => {
  ...
  it('renders exactly one non-ghost panel at a time', async () => {
    const wrapper = mountSettings()
    for (const label of ['Members', 'Danger Zone', 'About', 'Preferences']) {
